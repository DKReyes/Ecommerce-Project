class CheckoutController < ApplicationController
  def format_taxes(name, percentage, description)
    {
      ...
      price_data: {
        currency: "cad",
        unit_amount: (current_order.price_total * (percentage / 100)).to_i,
        product_data: {
  ...
  def success
    # Successfully take user's money
    @session = Stripe::Checkout::Session.retrieve(params[:session_id])
    @payment_intent = Stripe::PaymentIntent.retrieve(@session.payment_intent)
    @line_items = Stripe::Checkout::Session.list_line_items(params[:session_id], {limit: 99})

    current_order.update(price_total: @session.amount_total, payment: params[:session_id],
